...
- **Roadmap features shipped (2026-07-07):** A1 (multi-paste queue + `startBatchDigest` + `/api/batch/digest`), C2 (auto-tagging via dedicated `/api/tags/suggest`), C1 (ask-across-library RAG + `buildLibraryFtsQuery()` FTS5 OR-of-quoted-terms), B1 (vault.js markdown sync + `/api/vault/sync`), A2 (channel following with Inbox + local/desktop-only). B2 (TTS read-aloud) intentionally dropped by user request. All tests pass 204/204, security-scanned, runtime-verified on dev tree.
